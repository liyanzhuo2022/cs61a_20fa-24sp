...
class Account:
    ...
    def deposit(self, amount):
        ...
        """
        self.tran_times += 1
        self.transactions.append(Transaction(self.tran_times, self.balance, self.balance + amount))
    ...
    def withdraw(self, amount):
        ...
        if amount > self.balance:
            self.transactions.append(Transaction(self.tran_times, self.balance, self.balance))
...
def make_change(amount, coins):
    ...
    if amount < smallest:
        ...
        # check if it is None before adding list and (possiblily) Nonetype
        if result:
            # case 1: the smallest coin could be used
            return [smallest] + result
        else:
            # case 2: if the smallest coin is used, return None meaning no solution, so go to the next_small
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        if result:
            # pop out the used coins
            for c in result:
                self.coins = remove_one(self.coins, c)
            # add the new coin into machine
            ...
            try:
                ...
                self.coins[coin] = 1
            return result
        # if there is no solution, just pop out the new coin
        else:
            return [coin]
