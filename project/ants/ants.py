...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        "*** YOUR CODE HERE ***"
        # check if it is the first place
        if self.exit != None:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        disdance = 0
        while plc != beehive:
            if self.min_range <= disdance <= self.max_range and plc.bees != []:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # manipulate a bee for only once to reduce bugs
        for bee in bees:
            if armor <= 0:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        if self.digesting == 0 or self.digesting > self.time_to_digest:
            if self.place.bees != []:
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt should have name and food_cost as class attr"""
    name = 'Wall'
    food_cost = 4
    implemented = True
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        "*** YOUR CODE HERE ***"
        super().__init__(armor)
        if QueenAnt.init_called == False:
            self.is_queen = True
            QueenAnt.init_called = True
        else:
            self.is_queen = False
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        # when init_called == 1, it is the true queen
        if self.is_queen:
            super().action(gamestate)
            place = self.place.exit
            while place != None:
                if place.ant != None and place.ant.buffed == False:
                    place.ant.damage *= 2
                    place.ant.buffed = True
                place = place.exit
        # when init_called is not 1, it's the impostor
        else:
            x = 'impostor: reduce armor is called'
            print("DEBUG:", x)
