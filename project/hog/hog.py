...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    if num_rolls == 0:
        round_scores = free_bacon(opponent_score)
    else:
        round_scores = roll_dice(num_rolls, dice)
    # END PROBLEM 3
    return round_scores
...
def swine_align(player_score, opponent_score):
    ...
    gcd = 1
    i = 1
    while i <= player_score and i <= opponent_score:
        if player_score % i == 0 and opponent_score % i == 0 and i >= gcd:
            gcd = i
        i += 1
    # END PROBLEM 4a
    return gcd >= 10
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    # to show that it doesn't matter whether 'say' is used,
    # but use the same name in every round when call say function
    func = say
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    def count_high(new_score0, new_score1):
        """
        if who == 0:
        ...
        """
        if who == 0:
            new_score = new_score0
        else:
            new_score = new_score1
        cur_running_high = new_score - last_score
        if cur_running_high > running_high:
            print(f"{cur_running_high} point(s)! The most yet for Player {who}")
            return announce_highest(who, new_score, cur_running_high)
        else:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    highest_scores = 0
    num_rolls = 1
    # each number of rolls, from 1 to 10
    while i <= 10:
        scores = make_averaged(roll_dice, trials_count)(i, dice)
...
def bacon_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    bacon_scores = free_bacon(opponent_score)
    if bacon_scores >= cutoff:
        return 0
    return num_rolls
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    # code 1(wrong version)
    number = bacon_strategy(score, opponent_score, cutoff, num_rolls)
