...
def balanced(m):
    ...
    '''
    有点笨蛋的...
    torque_left = total_mass(end(right(m))) * length(right(m))
    torque_right = total_mass(end(left(m))) * length(left(m))
    if torque_left != torque_right:
        return False
    else:
        if is_mobile(end(right(m))) or is_mobile(end(left(m))):
        ...
        return True
    '''
    # base case: m is not mobile but planet!!!
    if is_planet(m):
        return True
    else:
        torque_left = total_mass(end(right(m))) * length(right(m))
        torque_right = total_mass(end(left(m))) * length(left(m))
        # using "and", if one gets False, the whole expression gets False!!!
        return torque_left == torque_right and balanced(end(right(m))) and balanced(end(left(m)))
